...
export async function listCalendarEvents(params: { startDate?: Date; endDate?: Date }) {
  try {
    ...
    return await getCalendarItemsForRange(start, end);
  } catch (err) {
    console.error("Error in listCalendarEvents:", err);
    return [];
  }
}
  ...
  try {
    ...
    for (const d of deliveries) {
      items.push({
        id: d.id,
        type: "delivery",
        title: `Delivery ${d.number} (${d.customer?.name || "Customer"})`,
        date: d.deliveryDate,
        status: d.status,
    ...
    console.error("Error in getCalendarItemsForRange:", err);
    return [];
  }
}
...
export async function getCalendarEventDetail(id: string) {
  try {
    ...
    console.error("Error in getCalendarEventDetail:", err);
    return null;
  }
}
...
export async function getPlannerDataForDate(date: Date) {
  try {
    ...
    console.error("Error in getPlannerDataForDate:", err);
    return { tasks: [], events: [], orders: [], deliveries: [], notes: [] };
  }
}
