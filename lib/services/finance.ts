...
export async function getFinanceOverview() {
  try {
    ...
    return {
      ...
      receivablesPaise: 0,
      expensesByCategory: [],
      invoiceSummary: { paid: 0, partiallyPaid: 0, unpaid: 0, overdue: 0, draft: 0 },
    };
  }
}
...
export async function getExpenseBreakdownByCategory(): Promise<ExpenseCategoryBreakdown[]> {
  try {
    ...
    console.error("Error in getExpenseBreakdownByCategory:", err);
    return [];
  }
}
...
export async function getRecentFinancialActivity(limit = 10): Promise<FinancialActivityItem[]> {
  try {
    ...
    console.error("Error in getRecentFinancialActivity:", err);
    return [];
  }
}
...
export async function getMonthlyRevenueThisYear() {
  try {
    ...
    console.error("Error in getMonthlyRevenueThisYear:", err);
    return [];
  }
}
